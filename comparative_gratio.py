...
def extract_max_fiber_diameters(df):
    ...
    for i in range(1, 7):  # Assuming there are 6 subsets
        for group_number in range(1, 4):  # Assuming there are 3 groups (CTL1, CTL2, CTL3)
...
def create_scatter_plot(df, max_fibers, title_prefix, group_prefix):
    ...
    fig, ax = plt.subplots()
    colors = ['red', 'green', 'blue']  # Colors for group 1, group 2, group 3
    g_ratio_means = []
    data_plotted = False
    ...
    for i in range(1, 7):  # Assuming there are 6 subsets
        for group_number in range(1, 4):  # Assuming there are 3 groups
            ...
            max_fiber_key = f"{group_number}_{i}"

            if g_ratio_mean_key in df.columns and g_ratio_std_key in df.columns and max_fiber_key in max_fibers:
                mean = df[g_ratio_mean_key].dropna().mean()
                std = df[g_ratio_std_key].dropna().std()
                ax.errorbar(max_fibers[max_fiber_key], mean, yerr=std, fmt='o', capsize=2,
                            color=colors[group_number-1], label=f"{group_prefix}{group_number}" if i == 1 else "")
                g_ratio_means.append(mean)
                data_plotted = True

    # Set the x-axis to span the range of max fiber diameters derived from CTL
